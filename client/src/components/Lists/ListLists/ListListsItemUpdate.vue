<template>
  <div class="list-up">
    <div class="list-header">
      <div class="list-save" @click="save">
    ...
    <div class="img">
      ...
      <img @error="isError" :src="preview" alt="">
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.preview= "https://localhost:7279/Files/"+this.list?.file?.fileName
  ...
  methods:{
    async save(){
      if(this.title == this.list.title && this.img==null){
        this.$emit('close');
      }

      let valid = ListValidator.UpdateListValidator(this.title, this.img);

      if(valid){
        return;
      }

      let response = await ListService.UpdateList(this.title,this.img, this.list.id);
      ...
      this.$emit('save', response.response.list);
      this.title=response.response.title;
      this.file=null;
      this.preview= "https://localhost:7279/Files/"+response?.response?.file?.fileName;
